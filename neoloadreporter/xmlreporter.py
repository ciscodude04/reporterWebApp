            ...
            for statlayer1 in vu.findall('statistic-item'):
                for statlayer2 in statlayer1.findall('statistic-item'):
                    for statlayer3 in statlayer2.findall('statistic-item'):
                        ...
                        sl390percentile = statlayer3.get('truncatedMean')
                        sl395percentile = statlayer3.get('percentile2')
                        sl399percentile = statlayer3.get('percentile3')
                        #print(f'{sl3scenario}, {sl3step}, {sl3count}')

                        data.append((project, testrun_name, sl3scenario, sl3step, sl3count,sl3errorcount, sl3min, sl3avg1,sl3max, sl350percentile, sl390percentile, sl395percentile, sl399percentile, sl3deviation,status, term_reason, duration, start_time, end_time))
...
def neoload_sql_xml_reader(table_name, xml_file_location):
    tree = ET.parse(xml_file_location)
    root = tree.getroot()
    test_data = xmlReader()
    mydata = test_data.fetchscenarioDatafromXML(root)
    ...
    query = mysqlobj.sql_statement_insert_data(table_name)
